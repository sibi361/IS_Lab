...
def encrypt(key, plain_text):
    ...
    for idx, character in enumerate(plain_text):
        if not first_iteration and idx != len(plain_text) - 1:
            key_char = cipher[idx - 1]
            key = ord(key_char)
...
def decrypt(key, cipher_text):
    ...
    for idx, character in enumerate(cipher_text):
        if not first_iteration and idx != len(cipher_text) - 1:
            key_char = cipher_text[idx - 1]
            key = ord(key_char)
            if key_char in string.ascii_lowercase:
                key -= 97
            elif key_char in string.ascii_uppercase:
